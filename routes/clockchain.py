...
def clockchain_view():
    """Display the Clockchain timeline view - PredictionMarketV2"""
    try:
...
def get_clockchain_events():
    """Get clockchain events for a specific time range - V2 Blockchain-Only"""
    try:
...
def submission_detail(submission_id):
    ...
    try:
        # V2: Redirect to clockchain view with message
...
@clockchain_bp.route('/api/contract-abi/<contract_name>')
...
@clockchain_bp.route('/api/admin/resolution-stats')
...
@clockchain_bp.route('/api/admin/pending-markets')
...
@clockchain_bp.route('/api/admin/market/<int:market_id>/details')
...
@clockchain_bp.route('/api/admin/resolve-market/<int:market_id>', methods=['POST'])
...
@clockchain_bp.route('/api/admin/auto-resolve-market/<int:market_id>', methods=['POST'])
...
@clockchain_bp.route('/api/admin/withdraw-fees', methods=['POST'])
...
@clockchain_bp.route('/clockchain/admin/resolution')
def resolution_dashboard():
    ...
    try:
        ...
        return render_template('clockchain/admin_resolution.html',
                             stats=stats,
                             pending_markets=pending)
    except Exception as e:
