...
Proteus is deployed on BASE (Coinbase L2, OP Stack) with the following stack:
...
The dominant strategy for text prediction in Proteus is to prompt a frontier large language model with a persona simulation request:
...
On-chain predictions are publicly visible. A public figure who monitors Proteus for their handle could game the system by deliberately deviating from predictions, penalizing AI predictors.
...
We have presented Proteus, a prediction market protocol that scores predictions by on-chain Levenshtein edit distance rather than binary yes/no resolution. The key properties are:
